...
@app.cell
def _(MODEL_NAME_MAPPING, MSESR_CORRECT_ANSWERS, pd):
    df_confidence = pd.read_pickle('../../03-processed_data/validations/task-4_accuracy/call4.pkl')
...
def _(List, pd):
    def group_compute_stats(df: pd.DataFrame, grouping_vars: List[str]) -> pd.DataFrame:
        ...
        # Scale the raw scores using the actual min and max found in the data
        df["confidence_scaled"] = (
            (df["confidence_score"] - df["confidence_score"].min()) / 
