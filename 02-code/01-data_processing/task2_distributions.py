...
def _(FOLDER_NAME_MAPPING, pd):
    df = pd.read_csv("../../03-processed_data/validations/task-2/call2_dataset.csv")
...
def _(anxiety_items, new_df, self_efficacy_items):
    anxiety_filter = (new_df["scale"] == "mseaq") & (new_df["item number"].astype(str).isin(anxiety_items))
    self_efficacy_filter = (new_df["scale"] == "mseaq") & (new_df["item number"].astype(str).isin(self_efficacy_items))
...
def _(Patch, Path, color_human, color_llm, gaussian_kde, np, plt):
    def create_custom_ridgeline(
        ...
        for i, model in enumerate(models):
            ...
            # --- HUMAN MODE (Blue) ---
            human_data = df_filtered[(df_filtered[model_col] == model) & (df_filtered[mode_col].str.lower() == "human")][score_col].dropna().values
            ...
            # --- LLM MODE (Orange) ---
            llm_data = df_filtered[(df_filtered[model_col] == model) & (df_filtered[mode_col].str.lower() == "llm")][score_col].dropna().values
        ...
        if save_path:
            if custom_name:
                ...
                plt.savefig(path.joinpath(str(custom_name)+"_joyplot.svg"), format = "svg")
...
def _(create_custom_ridgeline, df_viz):
    create_custom_ridgeline(df_viz, target_scale="amas", scale_col="scale", model_col="Model", mode_col="mode", score_col="sum_of_scores")
    return


@app.cell
def _(create_custom_ridgeline, df_viz):
    create_custom_ridgeline(df_viz, target_scale="maes", scale_col="scale", model_col="Model", mode_col="mode", score_col="sum_of_scores") 
    return


@app.cell
def _(create_custom_ridgeline, df_viz_anxiety):
    create_custom_ridgeline(df_viz_anxiety, target_scale="mseaq", scale_col="scale", model_col="Model", mode_col="mode", score_col="sum_of_scores") 
    return


@app.cell
def _(create_custom_ridgeline, df_viz_self_efficacy):
    create_custom_ridgeline(df_viz_self_efficacy, target_scale="mseaq", scale_col="scale", model_col="Model", mode_col="mode", score_col="sum_of_scores")
    return
